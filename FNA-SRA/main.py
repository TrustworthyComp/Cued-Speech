...
from fna_sra.t5_sra import FlanT5SLT  # noqa: F401 – ensure early import to avoid segfault
from fna_sra.callbacks import SetupCallback
...
def get_parser() -> argparse.ArgumentParser:
    ...
    """
    parser = argparse.ArgumentParser(description='FNA-SRA training and evaluation')
    parser.add_argument(
...
def configure_callbacks(
    ...
    if opt.evaluation == "bleu":
    ...
    elif opt.evaluation == "per":
        # Phoneme Error Rate – lower is better
        callbacks.append(ModelCheckpoint(
            dirpath=ckptdir,
            filename="epoch={epoch:05}-step={step:07}-per={val/per:.2f}",
            monitor=model.monitor,
            auto_insert_metric_name=False,
            save_top_k=1,
            mode="min"
        ...
        callbacks.append(EarlyStopping(
            monitor=model.monitor, verbose=True, patience=25, mode="min"
        ))
    elif opt.evaluation == "wer":
        # Word Error Rate (token-level for phoneme sequences) – lower is better
        callbacks.append(ModelCheckpoint(
            dirpath=ckptdir,
            filename="epoch={epoch:05}-step={step:07}-wer={val/wer:.2f}",
            monitor=model.monitor,
            auto_insert_metric_name=False,
            save_top_k=1,
            mode="min"
    ...
    elif opt.evaluation == "cer":
        # Character Error Rate – lower is better; filename also records WER
        callbacks.append(ModelCheckpoint(
            dirpath=ckptdir,
            filename="epoch={epoch:05}-step={step:07}-cer={val/cer:.2f}-wer={val/wer:.2f}",
            monitor=model.monitor,   # val/cer
            auto_insert_metric_name=False,
            save_top_k=1,
            mode="min"
        ))
...
def main():
    ...
    # Disable cuDNN to work around CUDNN_STATUS_NOT_INITIALIZED on
    # cuDNN 9.x + newer GPU architectures (Ada/Blackwell).
    # Performance impact is negligible since the model is dominated
    # by Transformer ops, not convolutions.
    import torch
