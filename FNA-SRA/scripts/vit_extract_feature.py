...
def read_video_frames(video_path):
    """Extract all frames from a video file using OpenCV, returning PIL Images."""
...
def get_parser():
    ...
    parser.add_argument('--anno_root', help='location of annotation npy files (not needed for MCCSD)', default=None)
    parser.add_argument('--video_root', help='location of video files', required=True)
    ...
    parser.add_argument('--ds_name', help='dataset name (MCCSD, Phoenix14T, CSL-Daily, How2Sign)', default='MCCSD')
    parser.add_argument('--signers', nargs='+', type=str, default=['LF', 'HS', 'WT', 'XP'],
                        help='MCCSD signer subsets to process')

    return parser


def get_iterator_legacy(args, mode):
    """Original iterator for Phoenix14T / CSL-Daily / How2Sign datasets."""
...
def process_mccsd(args):
    """Process the MCCSD dataset: scan video files by signer, extract frames, compute ViT features."""
    _model_name = os.path.split(args.model_name)[-1]
    feat_dir_name = f'{_model_name}_feat_mccsd'
    ...
    for signer in args.signers:
        signer_video_dir = osp.join(args.video_root, signer)
        if not osp.isdir(signer_video_dir):
            print(f"Warning: signer directory not found: {signer_video_dir}, skipping.")
            continue

        video_files = sorted(glob.glob(osp.join(signer_video_dir, '*.mp4')))
        if len(video_files) == 0:
            print(f"Warning: no .mp4 files found in {signer_video_dir}, skipping.")
            continue

        save_dir = osp.join(args.save_dir, feat_dir_name, signer)
        os.makedirs(save_dir, exist_ok=True)

        for video_path in tqdm.tqdm(video_files, desc=f'[{signer}]'):
            video_id = osp.splitext(osp.basename(video_path))[0]
            save_path = osp.join(save_dir, f'{video_id}{postfix}.npy')
            ...
            for j in range(0, len(frames), args.batch_size):
                frame_batch = frames[j:min(j + args.batch_size, len(frames))]
                feats = reader.get_feats(frame_batch).cpu().numpy()
                video_feats.append(feats)

            feats = np.concatenate(video_feats, axis=0)
...
def main():
    ...
    if args.ds_name == 'MCCSD':
        process_mccsd(args)
    else:
        # Legacy multi-split processing for Phoenix14T / CSL-Daily / How2Sign / NIASL2021
        mode = ["dev", "test", "train"]
        for m in mode:
            ds_name = args.ds_name
            _model_name = os.path.split(args.model_name)[-1]
            ...
            generator, num = get_iterator_legacy(args, _m)
            iterator = generator()
